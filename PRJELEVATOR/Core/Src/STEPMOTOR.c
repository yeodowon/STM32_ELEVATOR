...
int ext_floor;										// 외부 1층 버튼에 의해 저장되는 변수
...
{
	switch(motor_ext_state)
	{
		case BACKWARD :
			elevator_motor_call_down();
			elevator_led_down();
		break;
	}
}
...
void elevator_motor_call_down()
{
	if(INT_state == INT_inactive && DOOR_state == DOOR_CLOSE)
	{
		...
		else if(floor_count == STEPS_PER_REV * ext_floor)
		{
			...
			EXT_state = EXT_inactive;
			motor_ext_state = IDLE;
